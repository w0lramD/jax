...
def sync_global_devices(name: str):
  ...
  _assert_same(h, f"sync_global_devices name mismatch ('{name}')")


def process_allgather(in_tree: PyTreeDef) -> PyTreeDef:
  ...
  Returns:
    Pytress of arrays where the data is gathered from all hosts.
      ...
      * If the input is non-GDA, then the output is concatenated. So if each
        host has (2, 2) shaped arrays and there are 4 hosts, then after
        allgather, each host will have (8, 2) shaped arrays
...
def _assert_same(in_tree, fail_message: str = ''):
